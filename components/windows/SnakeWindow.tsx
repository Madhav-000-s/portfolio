...
// Size constraints for snake window
const MIN_WIDTH = 450
const MAX_WIDTH = 600
...
export default function SnakeWindow() {
  const snakeState = useWindowStore((state) => state.windows.snake)
  const focuswindow = useWindowStore((state) => state.focuswindow)
  const windowRef = useRef<HTMLDivElement>(null)

  // Resize state
  const [dimensions, setDimensions] = useState({ width: 480, height: 620 })
  ...
  useEffect(() => {
    if (!windowRef.current || !snakeState?.isOpen) return
  ...
  }, [snakeState?.isOpen, focuswindow])

  if (!snakeState?.isOpen) return null
