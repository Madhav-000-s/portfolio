...
// Size constraints for memory window
const MIN_WIDTH = 420
const MAX_WIDTH = 550
...
export default function MemoryWindow() {
  const memoryState = useWindowStore((state) => state.windows.memory)
  const focuswindow = useWindowStore((state) => state.focuswindow)
  const windowRef = useRef<HTMLDivElement>(null)

  // Resize state
  const [dimensions, setDimensions] = useState({ width: 450, height: 580 })
  ...
  useEffect(() => {
    if (!windowRef.current || !memoryState?.isOpen) return
  ...
  }, [memoryState?.isOpen, focuswindow])

  if (!memoryState?.isOpen) return null

  return (
    <div
      ref={windowRef}
      id="memory-game-window"
      style={{
        zIndex: memoryState.zIndex,
        width: dimensions.width,
        height: dimensions.height,
        top: "110px",
        left: "40%",
