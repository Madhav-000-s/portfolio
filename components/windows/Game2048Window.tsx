...
import Game2048 from "@/windows/game2048"
...
// Size constraints for 2048 window
const MIN_WIDTH = 350
const MAX_WIDTH = 500
const MIN_HEIGHT = 500
const MAX_HEIGHT = 700

export default function Game2048Window() {
  const game2048State = useWindowStore((state) => state.windows.game2048)
  ...
  const handleResizeStart = useCallback((direction: string) => (e: React.MouseEvent) => {
    ...
    focuswindow("game2048")
  }, [dimensions, focuswindow, handleResizeMove, handleResizeEnd])

  useEffect(() => {
    if (!windowRef.current || !game2048State?.isOpen) return
  ...
  }, [game2048State?.isOpen, focuswindow])

  if (!game2048State?.isOpen) return null

  return (
    <div
      ref={windowRef}
      id="game2048-game-window"
      style={{
        zIndex: game2048State.zIndex,
        width: dimensions.width,
        height: dimensions.height,
        top: "120px",
        left: "30%",
