...
// Size constraints for textfile window
const MIN_WIDTH = 400
const MAX_WIDTH = 900
...
export default function TextFileWindow() {
  ...
  // Check if file should open at max size
  const shouldMaxSize = txtfileState?.data?.maxSize
  ...
  useEffect(() => {
    if (!windowRef.current || !txtfileState?.isOpen) return
  ...
  }, [txtfileState?.isOpen, focuswindow])

  if (!txtfileState?.isOpen) return null
